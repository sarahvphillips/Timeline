...
- [x] **People join codes** (17 Sep 2026): Invite creates Firestore `joinInvites/{CODE}` and a `timelineapp://join/CODE` link. Friend signs in → Home → Enter invite code (or open the link) → Accept. Owner People list flips to **Uses Timeline** on next open (`syncAcceptedJoins`). **Publish `firestore.rules` joinInvites block in Firebase Console.**
